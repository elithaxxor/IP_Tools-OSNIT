...
USER_NAME = gp.getpass(prompt='[SYS] Enter username: : ', stream=None)
PASS = gp.getpass(prompt='[SYS] Enter password: : ', stream=None)
SERVER.login(USER_NAME, PASS)
RECEIVER = input(f'[SYS] Email:' )
SUBJECT = input(f'[SYS] Subject:' )
FAKE_NAME = input('[SYS] Spoof ID:')
logger = logging.getLogger()
logger.addHandler(smtp_handler)

smpt_handler = logging.handlers.SMPTPHandler(mailhost=SERVER,
                            fromaddr=[FAKE_NAME],
                            toaddrs=[RECEIVER],
                            subject=[SUBJECT],
                            credentials=(USER_NAME,PASS),
                            secure=None)
...
try:
    while True:
        ...
        with open('mail_01.txt', 'r') as f:
            ...
            f.close()
        print(f'[SYS Loop Resting for 10 seconds to prevent spam filter'])
        time.sleep(10)
        continue
...
text = msg.as_string()
server.sendmail('adel.alaali1990@gmail.com', 'adel.alaali@icloud.com', text)
